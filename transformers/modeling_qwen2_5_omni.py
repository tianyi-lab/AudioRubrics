...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def forward(
        ...
        if feature_attention_mask is not None:
            bs_audio = feature_attention_mask.shape[0]
            assert bs_audio == 1
        ...
        if inputs_embeds is None:
            ...
            if input_ids.shape[1] != 1:
                if input_features is not None:
                    ...
                    audio_features = audio_outputs.last_hidden_state # seq x hidden_size
                    ...
                    # 直接在这里尝试复制n份 seq -> n*seq, n = inputs_embeds.shape[0]
                    audio_features = audio_features.repeat(inputs_embeds.shape[0],1)
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            image_nums, video_nums, audio_nums = self._get_image_nums_and_video_nums(input_ids)
        ...
        def _expand_dict_for_generation(dict_to_expand):
            for key in dict_to_expand:
                if (
                    ...
                    and key not in audio_keys
                ):
                    dict_to_expand[key] = dict_to_expand[key].repeat_interleave(expand_size, dim=0)
            return dict_to_expand
        
        def _expand_dict_for_generation_audio(dict_to_expand):
            for key in dict_to_expand:
                if (
                    key != "cache_position"
                    and dict_to_expand[key] is not None
                    ...
                    dict_to_expand[key] = dict_to_expand[key] # do nothing, we repeat during forward
            return dict_to_expand
    ...
    def _get_image_nums_and_video_nums(
        ...
        audio_start_mask = input_ids == audio_start_token_id
        ...
        if audio_start_mask.sum() > 0:
            audio_nums = audio_start_mask.sum() 
            # 判断只能有image 或 video 不能同时有
            assert image_mask.sum()*video_mask.sum() == 0, "image and video cannot exist at the same time"
            video_nums = vision_first_mask.sum(1) if video_mask.sum() > 0 else 0*vision_first_mask.sum(1)
            image_nums = vision_first_mask.sum(1) if image_mask.sum() > 0 else 0*vision_first_mask.sum(1)
            # 纯音频(无 image/video)时 vision_first_mask.sum()==0, 该断言不应触发
            if image_mask.sum() > 0 or video_mask.sum() > 0:
                assert vision_first_mask.sum() == 1, "vision_first_mask sum should be 1"
            return image_nums, video_nums, audio_nums
